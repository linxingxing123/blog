...
func InitLogger(cfg *config.LogConfig) (err error) {
	ws := getLogWriter(cfg.Filename, cfg.MaxSize, cfg.MaxBackups, cfg.MaxAge) //日志写入文件
	encoder := getEncoder()                                                   // 日志输出的格式
	var level = new(zapcore.Level)                                            // 日志级别
	err = level.UnmarshalText([]byte(cfg.Level))                              // 解析日志级别
	if err != nil {
		return
	}
	...
	Logger = zap.New(core, zap.AddCaller(), zap.AddCallerSkip(1)) //创建日志记录器
	return
}
